...
public class AnalysisHelper {
    ...
    public void getMaxLikeCommentPost() {
        ...
        Comment commentMaxLikes = null;
        
        for(Comment c : data.getComments().values()) {
            if(commentMaxLikes == null)
                commentMaxLikes = c;
            if(c.getLikes() > commentMaxLikes.getLikes())
                commentMaxLikes = c;
        }
        int postId = commentMaxLikes.getPostId();
    ...
    public void getPassiveAndActiveOverallUsers() {
        ...
        for(int i=0; i<5; i++)
            System.out.println(users.get(users.size()-1-i)+ " - Overall count: "+ overallNumbers.get(users.get(users.size()-1-i).getId()));    
